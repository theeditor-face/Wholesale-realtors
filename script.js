...
async function loadProperties() {
  ...
  try {
    // Fetch without ordering first to avoid column errors
    ...
    if (!res.ok || (data && data.code)) {
      ...
      grid.innerHTML = `<div class="loading-state"><p>⚠️ Supabase error: ${errMsg}</p><p style="font-size:0.75rem;margin-top:8px;color:#666">Check RLS is OFF on properties table</p></div>`;
      return;
    }
    ...
    // Sort by id descending (newest first) in JS instead of relying on DB ordering
